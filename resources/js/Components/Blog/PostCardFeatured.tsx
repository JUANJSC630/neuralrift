...
export default function PostCardFeatured({ post }: { post: Post }) {
    ...
    return (
        <Link
            href={`/blog/${post.slug}`}
            className="group block relative min-h-[280px] sm:min-h-[400px] rounded-[18px] overflow-hidden
                         bg-gradient-to-br from-nr-bg3 to-nr-surface
                         flex items-end border border-nr-accent/[0.18]
                         hover:border-nr-accent/40 transition-colors duration-300"
        >
            {/* Grid pattern bg */}
            <div
